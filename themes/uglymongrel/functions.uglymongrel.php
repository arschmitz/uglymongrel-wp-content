<?php
/** =Common Functions for uglymongrel Sites
************************************************************
************************************************************/
...
function jq_get_github_url() {
	...
	$github_prefix = 'https://github.com/arschmitz/' . get_stylesheet() . '/tree/master/' . $source_path[0];
...
function jq_logo_link() {
	// TODO: remove when blog.uglymongrel.com-theme is gone
	if ( !function_exists( 'uglymongrel_sites' ) ) {
		return '/';
	}

	$sites = uglymongrel_sites();
	return empty( $sites[ UGLYMONGREL_LIVE_SITE ][ 'logo_link' ] ) ? '/' :
		$sites[ uglymongrel_LIVE_SITE ][ 'logo_link' ];
}
